...
void MyServer::startListening()
{
    ...
    int lowat = MSG_LEN; //设置发送和接收的低水位值，都是MSG_LEN，只有发送或接受缓冲区大于MSG_LEN才触发\
    读写事件，accept返回的sock将会自动继承这个水位值,因为每条消息长度固定为MSG_LEN
    ...
    while(!stop){
        ...
        for (int i = 0; i < fds; i++){ //遍历就绪事件 
            ...
            if (sockfd == listenfd && events[i].events & EPOLLIN){ //新连接
                ...
                char tmpBuff[ MSG_LEN ]; //用于临时缓存接收到的信息
                memset(tmpBuff, '\0', MSG_LEN);
                ret = recv(sockfd, tmpBuff, MSG_LEN, 0); //接收固定长度为MSG_LEN的信息
                ...
                if(sendNum >= userCounter) //已发送userCounter次，则分发结束，将isSending状态置false；
                    isSending = false;
                int idx = fd2IdxMap[sockfd];//该fd在clientfds数组对应的对应的idex
                ClientData* data = clientfds[idx];//得到fd对应的ClientData
                if(! data->writeBuf){
